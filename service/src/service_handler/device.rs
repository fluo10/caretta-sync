...
impl DeviceService for DeviceServiceHandler {
    ...
    async fn ping(
        ...
        let mut stream = self.context.as_ref().discover(public_key).await.ok_or(tonic::Status::not_found("Target peer address not found"))?;
        while let Some(x) = stream.next().await {
            let discovered = x.map_err(|e| tonic::Status::from_error(Box::new(e)))?;
            
            let result = iroh_ping::Ping::new().ping(self.context.as_ref().as_endpoint().unwrap(), discovered.into_endpoint_addr()).await.map_err(|e|tonic::Status::internal("ping error"))?;
