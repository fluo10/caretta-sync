...
mod tests {
    ...
    use crate::{data::local::{entity::{authorization_request, received_authorization_request, sent_authorization_request}, migration::TestMigrator, RemoteNodeActiveModel}, tests::TEST_CONFIG};
...
;
        ...
        let authorization_request = super::super::authorization_request::ActiveModel::new_test(&remote_node_model).insert(db).await.unwrap();

        let received_authorization_request_active_model = ActiveModel::new(&authorization_request);
        let received_authorization_request_model = received_authorization_request_active_model.clone().insert(db).await.unwrap();

        assert_eq!(received_authorization_request_active_model.sender_note.unwrap(), received_authorization_request_model.sender_note);
