...
use caretta_sync::{
    config::IrohConfig, global::IROH_ENDPOINT, proto::api::{device::{device_service_server::DeviceServiceServer, DeviceServer}, device_verification::{device_verification_service_server::DeviceVerificationServiceServer, DeviceVerificationServier}}, server::ServerTrait
};
...
impl ServerTrait for Server {
    ...
    where T: AsRef<caretta_sync::config::RpcConfig> + Send + Sync {
        let url = config.as_ref().endpoint_url.clone();
        let router =  tonic::transport::Server::builder()
            .add_service(DeviceServiceServer::new(DeviceServer))
            .add_service(DeviceVerificationServiceServer::new(DeviceVerificationServier));
        match url.scheme() {
            "unix" => {
