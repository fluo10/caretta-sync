...
fn extract_idents_from_fields_named_with_attribute<'a>(
...
pub fn extract_idents_and_types_from_data_struct_with_attribute<'a>(
    data: &'a DataStruct,
    attribute: &'static str,
) -> Vec<(Ident, Type)> {
...
pub fn extract_idents_and_types_from_data_struct<'a>(data: &'a DataStruct) -> Vec<(Ident, Type)> {
    ...
    fields
        .named
        .iter()
        .map(|x| (x.ident.clone().unwrap(), x.ty.clone()))
        .collect()
}
