...
async fn p2p_config() -> &'static P2pConfig {
    P2P_CONFIG
        .get_or_init(|| async move { P2pConfig::from(device_config().await.clone()) })
        .await
}
...
pub async fn database() -> &'static Database {
    DATABASE
        .get_or_init(|| async move { storage_config().await.open_database().await })
        .await
}
...
async fn device_config() -> &'static device_config::Model {
    DEVICE_CONFIG
        .get_or_init(|| async move {
            device_config::Model::get_or_try_init(database().await)
                .await
                .unwrap()
        })
        .await
