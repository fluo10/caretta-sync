...
impl Client {
    pub  async fn connect(url: &str, client_info: Implementation) -> Result<Self, crate::error::Error> {
...
impl Api for  Client {
    ...
    async fn device_ping(&self, params: DevicePingRequest) -> Result<DevicePingResponse, Self::Error> {
        ...
        let result = tool_result.into_typed().map_err(|e| ClientError::DeserializeResponse(e))?;
        Ok(result)
    }
