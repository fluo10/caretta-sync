...
impl TryFrom<ParsedMcpConfig> for McpConfig {
    ...
    fn try_from(value: ParsedMcpConfig) -> Result<Self, Self::Error> {
        let endpoint_url = value
            .endpoint_url
            .ok_or(ParsedConfigError::MissingConfig("endpoint_url"))?;
        let listen_addr = value
            .listen_addr
            .ok_or(ParsedConfigError::MissingConfig("listen_addr"))?;
        Ok(Self {
            endpoint_url,
            listen_addr,
...
impl Emptiable for ParsedMcpConfig {
    ...
    fn is_empty(&self) -> bool {
        self.endpoint_url.is_none() && self.listen_addr.is_none() && self.access_token.is_none()
    }
}
