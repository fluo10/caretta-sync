...
impl RunnableCommand for DevicePingCommandArgs {
    ...
    async fn run(self, app_info: AppInfo) {
        ...
        let client = self
            ...
            .into_client_config(app_name,self.verbose).unwrap()
            .spawn_client(app_info.client_info).await;
        let response = client.device_ping(crate::mcp::model::DevicePingRequest { target: self.target.into()}).await.unwrap();
